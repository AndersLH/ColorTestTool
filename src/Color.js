...
function Color({
	...
	function calcSRGB(x, y, conLine) {
		...
		let srgb = xyy2srgb(
			x / 100,
			1 - y / 100,
			maxBrightConfusion.current[conLine],
			brightReduce
		); //sliderBright old parameter
		...
		if (listConfusionColors.current.length === globalNumColors) {
			listConfusionColors.current[
				listConfusionColors.current.length + 1
			] = 1;
	...
	function isPointInTriangle(x, y) {
		...
		const totalArea = triangleArea(
			sRGBTriangle1976.a.x,
			sRGBTriangle1976.a.y,
			sRGBTriangle1976.b.x,
			sRGBTriangle1976.b.y,
			sRGBTriangle1976.c.x,
			sRGBTriangle1976.c.y
		);
	...
	function calcConfusionLine(i, min, max, type) {
		...
		if (
			i === numConfusionLines - 2 &&
			numConfusionLines > 1 &&
			type === "p"
		) {
			//Calculate the continuation of the line after the energy equal point
			const slope =
				(energyPoint1976.y - protan.y1) /
				(energyPoint1976.x - protan.x1);
		...
		if (
			...
			//Calculate the continuation of the line after the energy equal point
			const slope =
				(energyPoint1976.y - deutan.y1) /
				(energyPoint1976.x - deutan.x1);
		...
		if (
			...
			//Calculate the continuation of the line after the energy equal point
			const slope =
				(energyPoint1976.y - tritan.y1) /
				(energyPoint1976.x - tritan.x1);
	...
	function interpolate(x1, y1, x2, y2, t, i, j, radius, noise, type) {
		...
		for (let a = 0; a < j; a++) {
			while (
				Math.sqrt(
					(dot.x - listConfusionDots.current[i][a][0].x) ** 2 +
						(dot.y - listConfusionDots.current[i][a][0].y) ** 2
				) <= radius ||
				Math.sqrt(
					(dot.x - whitePoint1976.x) ** 2 +
						(dot.y - whitePoint1976.y) ** 2
				) <= 0.03
			) {
				t += 0.003;
				dot = mathInter(x1, y1, x2, y2, t, j);
	...
	function calcConfusionDot(x1, y1, x2, y2, stat, xCoor, i, j, type) {
		...
		let tValue = 0.1;
		if (xCoor) {
			//Checks if x2,y2 coordinates need to be swapped depending on the confusion line, tritan differs from deutan and protan
			dot = interpolate(
				x1,
				y1,
				calcConfusionLine(i, x2, y2, type),
				stat,
				tValue,
				i,
				j,
	...
	useEffect(() => {
		...
		if (currentRadio > 0 && document.getElementById("mm" + currentRadio)) {
			document
				.getElementById("mm" + currentRadio)
				.dispatchEvent(new MouseEvent("click", { bubbles: true }));
		}
	...
	return (
		<div>
			<div style={{ float: "left" }}>
				<p>
					(Note: Colors in the table are not accurate, no brightness
					applied)
				</p>
				...
				<table>
					<tbody>
						{Array.from({ length: globalNumColors + 1 }, (_, i) => (
          ...
          {toggleTable ? "ON" : "OFF"}
			...
			>
				...
				>
					...
					<image
						href={chromaticityImage}
						x={"7.3%"}
						y={"39.3%"}
						style={{ filter: `brightness(${sliderBright}%)` }}
						width="53.4%"
					/>

					{/* Dynamic creation of confusion lines */}
					{/* Protan */}
					{Array.from({ length: numConfusionLines }, (_, i) => {
							...
							return (
								<React.Fragment key={i + "pline"}>
									...
									<line
										...
										y1={`${
											10 + (100 - 100 * protan.y1) * 0.8
										}%`}
										x2={`${
											10 + 100 * protan.xyStop * 0.8
										}%`}
										y2={`${
											10 +
											(100 -
									...
									{Array.from(
										...
										(_, j) => {
											...
											const calcDot = calcConfusionDot(
												protan.x1,
												protan.y1,
												protan.yMin,
												protan.yMax,
							...
							return (
								<React.Fragment key={i + "dline"}>
									...
									<line
										...
										y1={`${
											10 + (100 - 100 * deutan.y1) * 0.8
										}%`}
										x2={`${
											10 + 100 * deutan.xyStop * 0.8
										}%`}
										y2={`${
											10 +
											(100 -
					...
					{/* x-axis */}
					<text
						style={{ fontSize: "19", textAnchor: "middle" }}
						x="50%"
						y="98%"
					>
						x
					...
					<circle
						...
						r={"2"}
						cx={`${10 + 100 * whitePoint1976.x * 0.8}%`}
						cy={`${10 + (100 - 100 * whitePoint1976.y) * 0.8}%`}
					/>
					{/* Energy point */}
					<circle
						style={{ stroke: "red", strokeWidth: "2" }}
						r={"2"}
						cx={`${10 + 100 * energyPoint1976.x * 0.8}%`}
						cy={`${10 + (100 - 100 * energyPoint1976.y) * 0.8}%`}
					/>

					{/* Possibly inaccurate sRGB triangle, find good source */}
					<line
						style={{ stroke: "black", strokeWidth: "2" }}
						x1={`${10 + 100 * sRGBTriangle1976.a.x * 0.8}%`}
						y1={`${10 + (100 - 100 * sRGBTriangle1976.a.y) * 0.8}%`}
						x2={`${10 + 100 * sRGBTriangle1976.b.x * 0.8}%`}
						y2={`${10 + (100 - 100 * sRGBTriangle1976.b.y) * 0.8}%`}
					/>
					<line
						style={{ stroke: "black", strokeWidth: "2" }}
						x1={`${10 + 100 * sRGBTriangle1976.b.x * 0.8}%`}
						y1={`${10 + (100 - 100 * sRGBTriangle1976.b.y) * 0.8}%`}
						x2={`${10 + 100 * sRGBTriangle1976.c.x * 0.8}%`}
						y2={`${10 + (100 - 100 * sRGBTriangle1976.c.y) * 0.8}%`}
					/>
					<line
						style={{ stroke: "black", strokeWidth: "2" }}
						x1={`${10 + 100 * sRGBTriangle1976.c.x * 0.8}%`}
						y1={`${10 + (100 - 100 * sRGBTriangle1976.c.y) * 0.8}%`}
						x2={`${10 + 100 * sRGBTriangle1976.a.x * 0.8}%`}
						y2={`${10 + (100 - 100 * sRGBTriangle1976.a.y) * 0.8}%`}
					/>
				</svg>
			</div>
