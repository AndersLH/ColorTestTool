...
function ColorTest() {
	...
	//AFK timer
	const timeoutAfk = useRef(100000000);
	let afkTimer = useRef(false);

	//End screen timer
	const timeoutEndScreen = useRef(3000000);
	...
	function placeCircles() {
		// Algorithm that places circles (points) in the plate

		figures.current = []; // Clears container with circle objects
		for (let i = 0; i < numberOfFigures; i++) {
			// Tries many times to place circle
			let canPlace = true; // Boolean flag telling if colliding or not
			let r =
				(plate.current.getAttribute("width") / 2) *
				Math.sqrt(Math.random()); // Random radius from center of plate
			...
			let c = new Circle(
				((r - Cr) * Math.cos(t) +
					...
					0,
				((r - Cr) * Math.sin(t) +
					plate.current.getAttribute("height") / 2) |
					0,
				Cr
			);

			for (let j = 0; j < figures.current.length; j++) {
				// Go through all circles currently placed in plate
				if (
					figures.current[j].r + c.r + globalMaxDistance >=
					getDistance(c, figures.current[j])
				) {
					// Collision with circle
					switch (
						c.r // Try through radius sizes until not colliding
					) {
						case MinRadius:
							canPlace = false;
							break;
	...
	function setColor() {
		// Gives colors to figures generated
		if (currentMotive.current) {
			// If a motive is set
			for (let i = 0; i < figures.current.length; i++) {
				...
				if (
					currentMotive.current[
						(figures.current[i].y /
							(plate.current.getAttribute("height") / motiveH)) |
							0
					][ // Is Circle within motive?
						(figures.current[i].x /
							(plate.current.getAttribute("width") / motiveW)) |
							0
					]
				)
					...
					if (activeTest.current === "noiseColor") {
						figures.current[i].fill =
							"fil" +
							(Math.random() < 0.11
								? rnd2 + globalNumColors - 1
								: rnd + globalNumColors);
					} else {
						figures.current[i].fill =
							"fil" + (rnd + globalNumColors); //
					}
				}
			}
		}
	...
	function getTextMap(text) {
		// Makes a motive from a text snippet (1-2 characters (Utf-8))
		...
		ctx.fillText(
			text,
			text.length > 1 ? motiveW / 10 : motiveW / 3,
			motiveH - motiveH / 5,
			motiveW
		);
		...
		for (let i = 0; i < canvas.height; i++) {
			...
			for (let j = 0; j < canvas.width; j++) {
				array[i][j] = canvasData.data[i * canvas.height * 4 + j * 4 + 3]
					? 1
					: 0;
			}
		}
	...
	function drawSVG() {
		...
		for (let c = 0; c < selectedFills.length; c++) {
			// Defines new fill colors to the new style element
			if (globalCurrentType !== "Ring" || selectedFills[c].id === "SC1")
				style.innerHTML +=
					"." +
					selectedFills[c].id +
					"{fill: " +
					selectedFills[c].value +
					"}\n";
			else
				style.innerHTML += `.${
					selectedFills[c].id
		...
		backgroundCircle.setAttribute(
			"cx",
			(plate.current.getAttribute("width") / 2) * 10
		);
		backgroundCircle.setAttribute(
			"cy",
			(plate.current.getAttribute("height") / 2) * 10
		);
		backgroundCircle.setAttribute(
			"r",
			(plate.current.getAttribute("height") / 2) * 10
		...
		for (let i = 0; i < figures.current.length; i++) {
			...
			if (
				...
				figure.setAttribute(
					"r",
					(figures.current[i].r + globalRadiusChange < 1
						? 0
						: figures.current[i].r + globalRadiusChange + ring) * 10
				);
			} else if (globalCurrentType === "Rect") {
				...
				figure.setAttribute(
					...
					(figures.current[i].r + globalRadiusChange < 1
						? 0
						: figures.current[i].r + globalRadiusChange) * 10
				);
				figure.setAttribute(
					"height",
					(figures.current[i].r / 2 + globalRadiusChange / 2 < 1
						? 0
						: figures.current[i].r / 2 + globalRadiusChange / 2) *
						10
				);
				figure.setAttribute(
					"transform",
					"rotate(" +
						(figures.current[i].angle * 180) / Math.PI +
						"," +
						(Number(figure.getAttribute("x")) +
							Number(figure.getAttribute("width")) / 2) +
						"," +
						(Number(figure.getAttribute("y")) +
							Number(figure.getAttribute("height")) / 2) +
						")"
				);
			} else if (globalCurrentType === "Square") {
				let size =
				...
				let size =
					(figures.current[i].r + globalRadiusChange < 1
						? 0
						: figures.current[i].r + globalRadiusChange) * 10.5;
	...
	function showColorChoice() {
		...
		for (let i = 0; i < globalNumColors * 2; i++) {
			...
			root.render(
				<Color
					globalNumColors={globalNumColors * 2}
			...
			input2.addEventListener("input", function () {
				...
				if (/^[a-f0-9 ]*$/i.test(this.value)) {
					//Checks for valid hex match
					document.getElementById(this.id.slice(0, -6)).value =
						"#" + (this.value.length < 6 ? "000000" : this.value);
					drawSVG();
			...
			if (i === globalNumColors * 2 - 1) {
				...
				for (let j = 1; j <= globalNumSpecialColors; j++) {
					...
					input.className = "fillColorsClass";
					input.addEventListener("input", function () {
						document.getElementById(this.id + "_label").value =
							this.value.replace("#", "");
						drawSVG();
					});
	...
	function exportToSVGFile() {
		...
		a.href =
			...
			window.btoa(svgCircles.current.innerHTML);
	...
	function fullReset() {
		...
		progressBar.current.innerHTML =
			"Progress: " + progressCount.current + " %";

		//Collect data after test
		document.getElementById("excelButton").click();

		//Update state of final scores
		setFinalScoreParticipantP(
			(prevMax) => prevMax + scoreParticipantP.current
		);
		setFinalScoreParticipantD(
			(prevMax) => prevMax + scoreParticipantD.current
		);
		setFinalScoreParticipantT(
			(prevMax) => prevMax + scoreParticipantT.current
		);
		...
		const handleKeyDown = (event) => {
			...
			if ("." === key) {
				wrapper.current.style.display =
					wrapper.current.style.display === "none" ? "flex" : "none";
				progressBar.current.style.display =
					wrapper.current.style.display === "none" ? "block" : "none";
				instructionPlate.current.style.display =
					wrapper.current.style.display === "none" ? "block" : "none";
				return;
			}
			...
			plateTime.current =
				(endPlateTime.current - startPlateTime.current) / 1000;
			...
			dataExcel.current.push({
				...
				NoiseLevel:
					activeTest.current === "noise" ? noiseLevel.current : "",
				NoiseColor: activeTest.current === "noiseColor" ? "true" : "",
				CircleSize:
					activeTest.current === "size" ? globalRadiusChange : "",
				Border:
					activeTest.current === "border"
						? globalBorder
							? "true"
							: "false"
						: "",
			...
			if (correctPress) {
				switch (currentColorType) {
					case "protan":
						scoreParticipantP.current += 1;
						break;
					case "deutan":
						scoreParticipantD.current += 1;
						break;
					case "tritan":
						scoreParticipantT.current += 1;
						break;
					default:
						break;
				}
				...
				if (
					chars[randomIndex] === motive.current.value &&
					randomIndex > 0
				) {
					randomIndex -= 1;
				...
				//Reached final test
				if (
					currentRadio === numConfusionLines &&
					currentColorType === "tritan"
				) {
					//Update progress
					...
					switch (activeTest.current) {
						...
						case "noise":
							...
							break;
						case "border":
							activeTest.current = "noiseColor";
							recieveColor("protan");
							recieveRadio(1);
							break;
						case "noiseColor":
							activeTest.current = "shape";
							recieveColor("protan");
							recieveRadio(1);
							globalCurrentType = "Triangle";
							globalRadiusChange = 6;
	...
	return (
		<div>
			...
			>
				...
				<div ref={startPage} className="startPage">
					...
					<p style={{ width: "40%", display: "inline-block" }}>
						This test will challenge your color vision, even if you
						think you have good color vision! As you take the test,
						it will adapt based on your performance. Simply use the{" "}
						<b>keyboard</b> to match the displayed{" "}
						<b>lower case letter or number</b> during the test.
						<br></br>
						<br></br>
						The test is anonymous and the data gathered will be used
						to aid my master's thesis. By participating in this
						test, you agree to your test results and information
						below being used and published in my master's thesis.
						<br></br>
						<br></br>
						Contact me at <b>andelha@stud.ntnu.no</b> if you have
						any questions.
						<br></br>
						<br></br>
					...
					<form ref={startForm}>
						...
						>
							...
							>
								<h3 className="rainbowTextDelay">Start test</h3>
								<button
									type="submit"
									onClick={(e) => {
										...
										//Hide start page
										startPage.current.style.display =
											"none";
										instructionPlate.current.style.display =
											"block";
										svgCircles.current.style.display =
											"block";
				...
				>
					<div style={{ display: "inline-block", textAlign: "left" }}>
						You have finished the test, thank you for participating!
						You got <br></br>
						<b>
							{finalScoreParticipantP}/{finalMaxScoreP}
						</b>{" "}
						correct answers for protan (red). <br></br>
						<b>
							{finalScoreParticipantD}/{finalMaxScoreD}
						</b>{" "}
						correct answers for deutan (green). <br></br>
						<b>
							{finalScoreParticipantT}/{finalMaxScoreT}
						</b>{" "}
						correct answers for tritan (blue). <br></br>
						<br></br>
						(Disclaimer: incorrect answers may not be an indication
						of color vision deficiency (color blindness), as the
						test is slighlty random making it prone to errors even
						for people with perfect color vision)
					</div>
					<br></br>
					<br></br>
				...
				<div ref={wrapper} style={{ display: "none", flex: "2" }}>
					<div ref={controls} style={{ display: "flex" }}>
						<fieldset style={{ flex: "1" }}>
							<label>Active test parameter: </label>
							<select
								onChange={(e) => {
							...
							>
								...
								<option value="noise">
									Noise (motive color in foreground)
								</option>
								<option value="border">Border</option>
								<option value="noiseColor">Color noise</option>
								<option value="shape">Shape</option>
								<option value="brightness">Brightness</option>
							</select>
							<hr></hr>
							<b>Change parameters manually:</b>
							<br></br>

							{/* Size */}
							<label>Size: </label>
							<button onClick={() => globalRadiusChange--}>
								-
							</button>
							<button onClick={() => (globalRadiusChange = 0)}>
								{" "}
								0
							</button>
							<button onClick={() => globalRadiusChange++}>
							...
							<input
								type="checkbox"
								onClick={() => (globalBorder = !globalBorder)}
							/>
							<br></br>
							...
							<select
								onChange={(e) => {
									globalCurrentType = e.target.value;
								}}
							>
								<option value="Circle" defaultValue={"Circle"}>
									Circles
								</option>
							...
							{/* Color noise */}
							<label>Confusion line noise:</label>
							<input
								type="number"
								defaultValue="0.000"
								min="0"
							...
							<b>Change specifications manually:</b>
							<label>Colors for motive/foreground:</label>
							<input
								type="number"
								defaultValue="3"
								min="1"
							...
							/>
							<button
								onClick={() => getTextMap(motive.current.value)}
							>
								Set new motive
							</button>
							<br />
							...
							<input
								...
								ref={numOfCircles}
								onInput={(e) =>
									(numberOfFigures = Number(e.target.value))
								}
							/>
							<label> figures </label>
							<br />
							...
							<button
								onClick={placeCircles}
								ref={placeCirclesButton}
							>
								Click to re-generate plate
							</button>

							<br />
							<div ref={generated}>Circles generated:</div>

							{/* Excel export */}
							<ExcelExport
								data={dataExcel.current}
								fileName="UserData"
							/>
							<br />

							{/* Div invisble and irrelevant for this test */}
							<div style={{ display: "none" }}>
								...
								<a ref={exportOfSVG} href="#">
									<button onClick={exportToSVGFile}>
										Download Plate
									</button>
								</a>
							</div>
							{/* Invisible div over for removing unused tools ^ */}
						</fieldset>
						<fieldset style={{ width: "33%", flex: "2" }}>
			...
			></canvas>
			<p
				ref={instructionPlate}
				style={{ display: "none", textAlign: "center" }}
			>
				Press the <b>letter or number</b> on the <b>keyboard</b> to
				match the motive. <br></br>
				If it is difficult to see, make your best guess. <br></br>
				If it is impossible to see, press <b>spacebar to skip</b> or
				take your best guess anyway. <br></br>
				<br></br>
				Press "." (punktum) to open and close configuration panel
			</p>
		</div>
	);
}
